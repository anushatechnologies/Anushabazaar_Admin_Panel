...
export default function DocumentReview() {
  ...
  const { data: pendingData } = useGetPendingDocumentsQuery();
  const {
    data: personsData,
  ...
  const {
    ...
    refetch: refetchDocs,
  } = useGetDeliveryPersonDocumentsQuery(selectedPersonId || 0, { skip: !selectedPersonId });
  const { data: personDetailData } = useGetDeliveryPersonByIdQuery(selectedPersonId || 0, {
    skip: !selectedPersonId,
  });

  const deliveryPersons = useMemo(
    () =>
      Array.isArray((personsData as any)?.deliveryPersons)
  ...
  const pendingDocuments = useMemo(
    () =>
      Array.isArray((pendingData as any)?.pendingDocuments)
  ...
  const personDocuments = useMemo(
    () =>
      Array.isArray((personDocsData as any)?.documents)
    ...
    [personDocsData, pendingDocuments, selectedPersonId],
  );
  ...
  const handleApprovePerson = async (personId: number) => {
    try {
      ...
      toast.error(
        err.data?.message || 'Account approval failed. Ensure all documents are approved.',
      );
    }
  };

  const selectedPerson = useMemo(
    () => deliveryPersons.find((p: any) => p.id === selectedPersonId),
    [deliveryPersons, selectedPersonId],
  );
  const selectedOnboarding = (personDetailData as any)?.onboardingStatus;
  const selectedOnboardingIssues = useMemo(() => {
    if (!selectedOnboarding) return [];

    return [
      !selectedOnboarding.personalInfoCompleted && 'personal info',
      !selectedOnboarding.vehicleCompleted && 'vehicle details',
      !selectedOnboarding.bankCompleted && 'bank details',
      !selectedOnboarding.profilePhotoApproved && 'profile photo approval',
      ...(selectedOnboarding.missingApprovedDocuments || []).map((code: string) =>
  ...
  const allDocsApproved = useMemo(() => {
    return !!selectedOnboarding?.readyForFinalApproval;
  }, [selectedOnboarding]);
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, minHeight: '100vh', background: currentTheme.bg }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              <Typography variant="h4" fontWeight={800} sx={{ mb: 0.5 }}>
                <GradientText>Delivery Person Verification</GradientText>
              </Typography>
              <Typography color="text.secondary">
                Review and approve delivery partner documents
              </Typography>
            </Box>
      ...
      >
        ...
        >
          ...
          <Tab
            icon={
              <Badge badgeContent={pendingDocuments.length} color="error">
                <PendingIcon />
              </Badge>
            }
            iconPosition="start"
            ...
            <Grid size={{ xs: 12, md: 8, lg: 9 }}>
              {selectedPerson ? (
                ...
                >
                  <GlassCard sx={{ minHeight: 'calc(100vh - 300px)' }}>
                    ...
                    <Box sx={{ p: 3 }}>
                      {selectedOnboarding && (
                        <Alert
                          severity={allDocsApproved ? 'success' : 'info'}
                          sx={{ mb: 3, borderRadius: 3 }}
                        >
                          {allDocsApproved
                            ? 'This partner has completed the required onboarding checks and is ready for final approval.'
                            : `Missing or pending required items: ${selectedOnboardingIssues.join(', ') || 'None'}`}
                          {!!selectedOnboarding.optionalDocuments?.length && (
                            <>
                              {' '}
                              Optional documents: {selectedOnboarding.optionalDocuments.join(', ')}.
                            </>
                          )}
                        </Alert>
                      )}
                          ...
                          {personDocuments.length === 0 ? (
                            <Grid size={{ xs: 12 }}>
                              <Box sx={{ textAlign: 'center', py: 8 }}>
                                ...
                                <Typography color="text.secondary">
                                  No documents available for this delivery person yet
                                </Typography>
                              </Box>
                            </Grid>
                          ) : (
                            personDocuments.map((doc: any) => (
                              <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={doc.id}>
                                ...
                                >
                                  <Box
                                    sx={{ position: 'relative', pt: '75%', background: '#f5f5f5' }}
                                  >
                                    <Box
                                      component="img"
                                      src={doc.documentUrl}
                ...
                <EmptyState
                  ...
                  description="Pick a delivery person from the left to review their files"
                />
              )}
            </Grid>
          </Grid>
              ...
              {pendingDocuments.length === 0 ? (
                <Grid size={{ xs: 12 }}>
                  <EmptyState
                    type="empty"
                    title="All Caught Up!"
                    description="There are no documents waiting for your review."
                  />
                </Grid>
              ) : (
                      ...
                      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        ...
                        <Box sx={{ flex: 1, overflow: 'hidden' }}>
                          <Typography variant="subtitle2" fontWeight={700} noWrap>
                            Person ID: {doc.deliveryPersonId}
                          </Typography>
      ...
      <Dialog
        ...
        PaperProps={{ sx: { borderRadius: 4, bgcolor: currentTheme.cardBg, p: 1 } }}
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            multiline
            rows={3}
            placeholder="e.g. Image blurry, invalid ID number"
            value={remarks}
