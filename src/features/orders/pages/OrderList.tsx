...
const OrderList: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (s === 'pending' || s === 'placed') return '#f59e0b';
          ...
          >
            <Box>
              <Typography variant="h4" fontWeight={900}>
                <GradientText>Orders Overview</GradientText>
              </Typography>
              <Typography color="text.secondary">Real-time order management dashboard</Typography>
            </Box>

            <GlassBadge statusColor={currentTheme.accent}>{orders.length} TOTAL ORDERS</GlassBadge>
          </Box>
        </GlassPageHeader>
      </motion.div>
                    ...
                    <TableRow>
                      <TableCell colSpan={5} align="center" sx={{ py: 10 }}>
                        <EmptyState
                          type="empty"
                          title="No Orders"
                          description="Wait for customers to place some orders."
                        />
                      </TableCell>
                    </TableRow>
                  )}
