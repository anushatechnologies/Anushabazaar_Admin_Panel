...
const ColorlibConnector = styled(StepConnector)(({ color }: any) => ({
...
const CustomerOrderTracking: React.FC = () => {
    ...
    if (order) {
      ...
      const steps = [
        ...
        },
        {
          label: 'Preparing',
          status: 'PROCESSING',
          completed: ['PROCESSING', 'READY', 'ASSIGNED', 'OUT FOR DELIVERY', 'DELIVERED'].includes(
            statusStr,
          ),
          icon: <RestaurantIcon />,
        },
        {
          ...
          completed:
            ['OUT FOR DELIVERY', 'DELIVERED', 'ASSIGNED', 'PICKED_UP'].includes(statusStr) &&
            statusStr !== 'READY' &&
  ...
  const activeStep = orderSteps.findIndex((s) => !s.completed);
  ...
  if (isError || !order)
    return (
      <Box p={4} textAlign="center">
        ...
        <Button onClick={() => navigate('/my-orders')} variant="contained" sx={{ mt: 2 }}>
          Back to Orders
        </Button>
      </Box>
    );
          ...
          >
            <Box>
              <Typography variant="h4" fontWeight={900}>
                <GradientText>Track Life Cycle</GradientText>
              </Typography>
            ...
            <Stack direction="row" spacing={2} sx={{ alignSelf: { xs: 'stretch', sm: 'center' } }}>
              <GlassBadge statusColor={currentTheme.accent}>
                ...
                {order.estimatedDeliveryTime
                  ? dayjs(order.estimatedDeliveryTime).format('hh:mm A')
                  : '--:--'}
              </GlassBadge>
            </Stack>
          </Box>
                ...
                <Step key={step.label}>
                  ...
                  >
                    <Typography
                      variant="subtitle2"
                      fontWeight={800}
                      sx={{ mt: 1, opacity: displayStep >= index ? 1 : 0.4 }}
                    >
                      {step.label}
                    </Typography>
                    {step.time && (
            ...
            <GlassCard sx={{ p: 0, overflow: 'hidden' }}>
              ...
              <Box sx={{ p: 3 }}>
                {order.items?.map((item, idx) => (
                  ...
                  >
                    <Stack direction="row" spacing={2} alignItems="center">
                      <Avatar
                        sx={{ bgcolor: `${currentTheme.accent}15`, color: currentTheme.accent }}
                      >
                        <OrderIcon />
                      </Avatar>
                      <Box>
                    ...
                    <Box textAlign="right">
                      ...
                      <GlassBadge sx={{ fontSize: '10px' }} statusColor="gray">
                        {item.quantity} UNITS
                      </GlassBadge>
                    </Box>
                  </Box>
                ...
                >
                  ...
                  <Typography variant="h4" fontWeight={900} color={currentTheme.accent}>
          ...
          >
            <Stack spacing={3}>
              <GlassCard sx={{ p: 3, border: `1px solid ${currentTheme.border}` }}>
                ...
                {(order as any).deliveryPersonName || order.deliveryPersonId ? (
                  <Stack direction="row" spacing={2} alignItems="center">
                    <Avatar sx={{ width: 60, height: 60, bgcolor: currentTheme.accent }}>
                      <DeliveryIcon sx={{ fontSize: 30 }} />
                    </Avatar>
                    <Box>
                      <Typography variant="h6" fontWeight={800}>
                        {(order as any).deliveryPersonName || 'Rider Assigned'}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        {(order as any).deliveryPersonPhone || 'On the way to pick up location'}
                      </Typography>
                    </Box>
                  </Stack>
                ) : (
