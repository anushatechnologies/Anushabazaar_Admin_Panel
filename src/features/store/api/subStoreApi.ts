...
export const subStoreApi = baseApiWithAuth.injectEndpoints({
  endpoints: (builder) => ({
    getSubStores: builder.query<PaginatedStoresResponse, { name?: string; page?: number; size?: number }>({
      query: (params = {}) => ({
        url: '/api/stores1',
        params: {
...
export const fetchSubStores = async (name?: string, page = 0, size = 10): Promise<PaginatedStoresResponse | SubStore[]> => {
...
export const createSubStore = async (storeData: SubStoreRequest, imageFile?: File): Promise<SubStore> => {
